def git_merge(path, from_branch, to_branch, merge_method, tag_version, deployment_script = nil)
  ...
  unless deployment_script.nil? || !deployment_script
    puts "Running the deployment script deployment_script".foreground(:blue)
    run_local <<-BASH
      cd #{path}
      ./#{deployment_script}
    BASH
